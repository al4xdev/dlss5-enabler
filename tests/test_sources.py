...
def test_fetch_feeder(tmp_path: Path, mocker: MockerFixture) -> None:
    ...
    assert bundle.release_tag == "v0.7.0"
...
def test_fetch_feeder_invalidates_cache_when_release_changes(tmp_path: Path, mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "dlss5_enabler.network.sources.http_get_json",
        side_effect=[{"tag_name": "v1.0.0"}, {"tag_name": "v2.0.0"}],
    )
    ...
    def write_download(_url: str, dest: Path | str, progress_fn: Any = None) -> Path:
        path = Path(dest)
        path.write_bytes(f"DOWNLOAD-{download.call_count}".encode())
        return path
    ...
    mock_releases: list[dict[str, Any]] = [
        {
            ...
            "assets": [
                {"name": "renodx-dlss5_4.70.zip", "browser_download_url": "http://example.com/renodx.zip"},
            ],
        },
    ]
    mocker.patch("dlss5_enabler.network.sources.http_get_json", return_value=mock_releases)
...
def test_fetch_ngx_dlls(tmp_path: Path, mocker: MockerFixture) -> None:
    ...
    mock_manifest: dict[str, Any] = {
        "dlssnr": [{"version": "310.8.SF-v2", "url": "http://example.com/nr.zip"}],
        "dlss": [{"version": "310.8.0", "url": "http://example.com/sr.zip"}],
    }
    mocker.patch("dlss5_enabler.network.sources.http_get_json", return_value=mock_manifest)
...
def test_fetch_reshade_headers(tmp_path: Path, mocker: MockerFixture) -> None:
    mocker.patch("dlss5_enabler.network.sources.get_cache_dir", return_value=tmp_path)
    mocker.patch("dlss5_enabler.network.sources.http_get_json", return_value={"default_branch": "slim"})
...
def test_fetch_lumenite(tmp_path: Path, mocker: MockerFixture) -> None:
    mocker.patch("dlss5_enabler.network.sources.get_cache_dir", return_value=tmp_path)
    mocker.patch("dlss5_enabler.network.sources.http_get_json", return_value={"default_branch": "main"})
...
def test_fetch_dgvoodoo(tmp_path: Path, mocker: MockerFixture) -> None:
    ...
    mock_release: dict[str, Any] = {
        "tag_name": "v2.87.3",
        "assets": [
            {"name": "dgVoodoo2_87_3.zip", "browser_download_url": "http://example.com/dgvoodoo.zip"},
        ],
    }
    mocker.patch("dlss5_enabler.network.sources.http_get_json", return_value=mock_release)
...
def test_fetch_dgvoodoo_uses_requested_architecture(tmp_path: Path, mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        ...
        return_value={
            "tag_name": "v2.87.3",
            "assets": [{"name": "dg.zip", "browser_download_url": "https://example.com/dg.zip"}],
        },
    )
